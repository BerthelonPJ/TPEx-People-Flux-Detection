...
#include "GestionWifi.h" // Librairie qui gère la liaison WiFi
...
GestionWifi gestionnaireWifi; // On instancie l'objet de gestion WiFi
...
void setup() {
  ...
  gestionnaireWifi = GestionWifi("esp-compteur", "88888888"); // Lancement du WiFi. avec les paramètres (SSID : esp-compteur et MDP : 88888888)
  
  gestionnaireWifi.launchAP(); // On lance le point d'accès WiFI

  gestionnaireWifi.launchServer(); // On lance le serveur WiFi
...
void loop() {
  ...
  gestionnaireWifi.handleCompteurRequest(compteur); // On gère les demandes de connexion au serveur Web
  delay(50); // delay pour eviter de looper trop vite
}
