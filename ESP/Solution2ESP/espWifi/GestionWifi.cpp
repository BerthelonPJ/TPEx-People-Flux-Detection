...
<HTML>\n\
<HEAD>\n\
<TITLE>Détecteur de flux</TITLE>\n\
</HEAD>\n\
<BODY>\n\
<h1 style=\"color: #5e9ca0;\"></h1>\n\
<p>&nbsp;</p>\n\
<p><strong>&nbsp;Compteur :&nbsp;</strong></p>\n\
...
void GestionWifi::handleCompteurRequest(int compteur){
    ...
    while (client.connected()){ // Si le client est connecté
      if (client.available()){  // et qu'il est disponible.
        ...
        if (lines.endsWith("\r\n\r\n")){ // Si le client veut le contenu de la page WEB.
          ...
          client.println(pageHTMLHead + compteur + pageHMTLEnd); // On concatène les différentes parties de la page web puis on l'envoie en entier au client.
